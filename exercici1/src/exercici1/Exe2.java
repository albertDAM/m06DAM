...
public class Exe2 {
	...
	public static void main(String[] args) {
        ...
        try (FileOutputStream fileOut = new FileOutputStream("players.txt")) {
        	DataOutputStream salido = new DataOutputStream(fileOut);
        	
        	for (int i=0;i<players.length;i++) {
        		for(int j=0;j<players[0].length;j++) {
        			salido.writeUTF(players[i][j]);
        		}
        	}
        	}catch (Exception e) {
        		System.out.println(e.toString());	
        	}
        try (DataInputStream fileIn = new DataInputStream(new FileInputStream("./players.txt"))) {
        	...
        	}catch (Exception a) THROWS IO.Exception {
